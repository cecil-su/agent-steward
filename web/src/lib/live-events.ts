...
export function subscribeLiveEvents(callbacks: LiveCallbacks, transport: typeof fetch = fetch) {
  ...
  void (async () => {
    while (!controller.signal.aborted) {
      ...
      try {
        const response = await transport('/api/events', { method: 'GET', headers: { 'X-Steward-UI-Contract': '4' }, credentials: 'same-origin', cache: 'no-store', redirect: 'error', signal: controller.signal });
