...
- Project提供名称/`##ID`、独立revision/History与Task projectId；当前源码数据库 Schema 7 仅初始化空库；非空数据库必须为 Schema 7，其他版本拒绝普通打开，不隐式迁移。组件/源码根登记、Task 组件范围及项目候选定位已实现，事实缓存尚未实现，见 [`docs/v0/16-项目与上下文复用.md`](docs/v0/16-项目与上下文复用.md)。
...
| Windows 启动器 | [`distribution/windows/steward.ps1`](distribution/windows/steward.ps1) | Schema 7 包、停止前数据库版本预检、同 Schema 回退及自定义目录入口；拒绝非7包，PID/启动时间/路径校验，不接管用户进程或自动跨 Schema 更新 |
...
| GUI | [`web/src`](web/src)、[`crates/server/src/ui.rs`](crates/server/src/ui.rs)、[`crates/server/web-readonly`](crates/server/web-readonly) | React 只读工作台；UiStore 托管兼容外置包，内嵌回退来自 web-readonly；UI API合同4、包格式1；web-legacy-readonly提供独立原生资源及图标 |
| Application 门面 | [`crates/application/src/lib.rs`](crates/application/src/lib.rs) | `Service`、`Outcome`、稳定错误映射、部分外部状态和恢复命令 |
| 项目资料 | [`crates/application/src/project_profiles.rs`](crates/application/src/project_profiles.rs) | 完整替换资料、Project revision及来源Task version校验、项目History；CLI维护、HTTP只读 |
| 个人/项目规则 | [`crates/application/src/rules.rs`](crates/application/src/rules.rs) | rule CLI、revision CAS、History、有效sessionRules上下文；不自动领取任务 |
...
| 项目管理 HTTP/页面 | [`crates/server/src/projects.rs`](crates/server/src/projects.rs)、[`web/src/features`](web/src/features) | 项目/组件/源码只读页面与任务筛选；HTTP保留独立Project/Task CAS命令，reader不可写；业务维护由CLI完成，页面不提供业务写入口 |
...
| v7 归档迁移 | [`crates/application/src/migration.rs`](crates/application/src/migration.rs) | 显式只读schema_migrations格式v7归档、闭合任务范围校验、逐字段复制核验、新库不覆盖发布；`database import-v7` |
| Schema 2 显式复制 | [`crates/application/src/migration/schema2.rs`](crates/application/src/migration/schema2.rs) | `database import-schema2`：冻结布局、七表/四序列复制、数据/身份复查、私有暂存及不覆盖发布；不探测源记录中的外部路径。失败/强杀测试在同名子目录，CLI 边界见 `crates/cli/tests/schema2_contract.rs`，真实启动重启见 `crates/server/tests/schema2_startup.rs` |
| Schema 4/5/6 显式复制 | [`crates/application/src/migration/schema2.rs`](crates/application/src/migration/schema2.rs)、[`crates/application/src/migration`](crates/application/src/migration) | import-schema4/5/6共用受控复制实现，各按冻结源SQL校验；目标Schema7，新规则为空，源不变，不切换服务 |
...
| Git 读取资源边界 | [`crates/git-adapter/src/read_process.rs`](crates/git-adapter/src/read_process.rs) | 所有Git读进程限10秒、stdout 8MiB/stderr 64KiB并清理受管理进程组/Job；Unix非阻塞pipe与停止标志避免逃逸writer阻塞reader回收，但不承诺杀死逃逸进程组；CLI here/context/source resolve/worktree status/doctor及HTTP读请求共用请求内总预算。Git写进程不套用读截止时间；无请求作用域的写前后观察各有独立读预算，保留部分外部状态分类 |
...
| --- | --- | --- | --- | --- |
| 项目/资料/源码 | `ProjectCommand` | `projects.rs/project_profiles.rs/sources.rs/project_context.rs` | SQLite、显式文件/Git观察 | `projects.rs`、`project_profiles.rs`、`sources.rs`、`project_context.rs` |
| 规则 | `RuleCommand` | `rules.rs` | SQLite | 各入口 `rules.rs` |
| Hook | `HookCommand` / task-hook | `hooks.rs` | SQLite、投影后的事件 | `hooks.rs`、`hook_adapter.rs` |
| 离线复制 | `DatabaseCommand` | `migration.rs/migration/schema2.rs` | 显式源快照、新目标库 | `migration.rs`、`migration_contract.rs`、`schema2_contract.rs`、`schema2_startup.rs` |
...
- Task 的 title/goal/scope/acceptanceCriteria 初始可空以支持最小创建和增量补全；设置为字符串后不可清空，`close completed` 前四项必须完整。新写入的 title 必须符合 `MMDD｜类型｜主题`；closed Task可按[信息维护合同](docs/v0/20-任务信息维护.md)执行经确认的update/project/components或retitle，不重开任务。
...
- 普通连接只支持Schema7，仅空库初始化，不隐式转换。`database import-v7`只读schema_migrations格式的闭合归档，并向不存在的新库发布校验后的副本，不改源、不合并、不自动切换配置。`database import-schema2/4/5/6` 各自支持对应冻结格式的停写快照（含活动任务），目标均为Schema7，不放宽普通连接的格式校验；一致快照与 data_version 复查不替代停止写入者。
- Task 的 Repository、common-dir、Branch、Worktree 四个引用必须全有或全空。
- Worktree锁仅对本次新建目录设置私有权限；已有lock_root须已私有且非链接，只验证、不改写调用方目录ACL或传播到后代。
...
- Worktree删除拒绝检查时观察到的staged、unstaged、untracked和ignored文件；检查与外部Git删除不原子，advisory lock不阻止其他写入者，不能承诺检查后新增ignored文件不被Git删除。系统不隐式执行 `push`、`force`、`clean`、`reset` 或 `stash`。
...
| [`crates/application/tests/projects.rs`](crates/application/tests/projects.rs)、[`crates/cli/tests/projects.rs`](crates/cli/tests/projects.rs) | 项目名/编号、两项目任务归属、CAS、History 回滚、分页、context Notes、源码CLI和不兼容Schema拒绝 |
...
| [`crates/server/tests/projects.rs`](crates/server/tests/projects.rs)、[`web/src/features/workspace.test.tsx`](web/src/features/workspace.test.tsx)、[`web/src/lib/api.test.ts`](web/src/lib/api.test.ts)、[`crates/server/tests/browser/embedded-readonly.test.cjs`](crates/server/tests/browser/embedded-readonly.test.cjs) | 项目HTTP/只读React契约、Project CAS、显式Git路径、无业务写传输入口和内嵌资源边界；不是实浏览器验收 |
| [`crates/application/tests/context_readonly.rs`](crates/application/tests/context_readonly.rs)、[`crates/application/tests/close_transition.rs`](crates/application/tests/close_transition.rs)、[`crates/application/tests/task_id_search.rs`](crates/application/tests/task_id_search.rs) | context只读无初始化、关闭转换、数字ID精确搜索 |
| 各入口 `pending_release.rs`、`rules.rs`、`task_maintenance.rs`；Application/CLI `project_profiles.rs` | 待上线状态、规则CAS/上下文、经确认的信息维护、资料来源及History回滚 |
| [`crates/cli/tests/input_boundaries.rs`](crates/cli/tests/input_boundaries.rs)、[`crates/cli/tests/worktree_reconciliation.rs`](crates/cli/tests/worktree_reconciliation.rs)、[`crates/server/src/resource_tests.rs`](crates/server/src/resource_tests.rs) | JSON边界、16MiB输入、无副作用分页校验、真实Git hook后的最新版本恢复、坏登记只读隔离与owner保守拒绝、缺失大小写别名拒绝detach、读写/认证容量与取消后permit保留 |
| [`crates/cli/tests/git_read_limits.rs`](crates/cli/tests/git_read_limits.rs)、[`crates/server/tests/local_access.rs`](crates/server/tests/local_access.rs)、[`crates/server/tests/ui_packages.rs`](crates/server/tests/ui_packages.rs) | CLI真实Git超时/输出限额、默认本机直连与严格认证边界、UI包版本兼容与回退 |
...
- `web/scripts/browser-smoke.mjs`：获准的临时Daemon/数据库/Git仓库真实浏览器检查，不能据脚本存在认定人工验收。
