...
function Read-UiPackage([string]$Directory) {
    ...
    if ($manifest.packageFormat -ne 1 -or $manifest.requiredApiContract -ne 5 -or $manifest.entry -cne 'index.html' -or $manifest.uiVersion -cnotmatch '^[A-Za-z0-9._-]{1,100}$') { throw 'Unsupported UI package or API contract.' }
...
function Build-UiPackage([string]$Repository, [string]$Destination, [string]$UiVersion) {
    ...
    Write-UiJson (Join-Path $Destination 'manifest.json') @{packageFormat=1;uiVersion=$UiVersion;requiredApiContract=5;entry='index.html';files=$hashes}
...
function Get-UiStatus([string]$Address) {
    ...
    if ($status.packageFormat -ne 1 -or $status.apiContract -ne 5 -or -not $status.externalEnabled) { throw 'taskd must first be upgraded and started with --ui-root. No service will be restarted by this script.' }
    return $status
}
