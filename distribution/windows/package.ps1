...
if ($LASTEXITCODE -ne 0 -or $result.Count -ne 1 -or $result[0] -cne 'databaseSchema=8') { throw 'Selected taskd does not match Schema 8; no package created.' }
...
Write-Json (Join-Path $bundle 'manifest.json') @{version=$Version;databaseSchema=8;launcherProtocol=1;uiPackageProtocol=1;target='x86_64-pc-windows-msvc'}
