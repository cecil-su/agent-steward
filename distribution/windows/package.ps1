...
if ($LASTEXITCODE -ne 0 -or $result.Count -ne 1 -or $result[0] -cne 'databaseSchema=5') { throw 'Selected taskd does not match Schema 5; no package created.' }
...
Write-Json (Join-Path $bundle 'manifest.json') @{version=$Version;databaseSchema=5;launcherProtocol=1;uiPackageProtocol=1;target='x86_64-pc-windows-msvc'}
