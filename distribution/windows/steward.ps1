...
function Assert-Manifest($Manifest) {
    ...
    if ($Manifest.databaseSchema -ne 8 -or $Manifest.launcherProtocol -ne 1 -or $Manifest.target -ne 'x86_64-pc-windows-msvc') {
        throw 'This launcher requires a Schema 8 package. Cross-schema upgrades require a separately backed-up migration and a separate installation directory.'
    }
}
