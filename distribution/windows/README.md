...
当前源码包仅接受 Schema8、UI合同5，CLI/HTTP JSON envelope为3；不代表已安装版本。Schema2/4/5/6/7安装不能通过普通更新进入Schema8，必须按[规则与迁移合同](../../docs/v0/22-个人偏好与项目规则.md)另行授权停写、备份、复制核验和独立安装。下方更新步骤仅适用于兼容Schema8安装或新的隔离安装，不可直接套用旧正式安装目录。

## 数据中心命令合同

- 任务状态为 `backlog`、`todo`（默认）、`in_progress`、`in_review`、`blocked`、`done`、`cancelled`，可任意转换；`active` 筛选排除 `done`/`cancelled`。
- `taskctl task status TASK STATUS --if-version VERSION` 独立修改状态，不领取、结束或替换 Session。同状态仍检查 CAS，成功后不增加版本或历史。
- HTTP 使用 `POST /api/commands/task-status`，正文为 `{"taskId":1,"expectedVersion":1,"status":"in_review"}`；保留权限、Origin 和 CSRF 校验。
- `task block/unblock/pending-release/continue/close/here`、`worktree`、`project here/context`、`project source resolve` 已移除，不提供兼容别名。`task context` 保持只读。
- `project source add PROJECT --directory ABSOLUTE_PATH --if-revision REVISION` 只登记普通目录资料；不读取源码、观察 Git 或管理 Worktree。
- Schema7 显式离线迁移：`taskctl --database TARGET --yes database import-schema7 --source SOURCE [--source-paths PATHS_JSON]`。映射文件为 `{"1":"E:/ai/project"}`（sourceId → 绝对目录）；省略时为 `{}`，旧 Git source 缺少映射时拒绝迁移。必须先停写并备份源库；目标为新库，不原地升级。Schema4/5/6 对应的 `import-schema4/5/6` 同样接受可选 `--source-paths`，默认 `{}`；必须使用真实源 Schema 对应命令，不修改源库版本号冒充 Schema7。
...
- 当前包及启动器只接受 **Schema8**。数据库和凭据不随程序替换，不自动迁移或恢复备份；已有 Schema 2/4/5/6/7 安装不走普通 Update/Update-Local，须先单独安排停写、最终一致备份及显式迁移，再使用独立安装目录，保留旧安装用于受控恢复。不会将旧程序自动回退到已经迁移的库上。
...
### 从 Schema 2/4/5/6/7 进入 Schema8

这是单独授权的维护操作，不是普通更新。按 [迁移合同](../../docs/v0/22-个人偏好与项目规则.md) 完成停写、备份、迁移及核验后，选择**新的独立 InstallRoot**，明确配置已核验的 Schema8 数据库和原 runtime 路径。数据库可以在获准后保留原正式路径，但程序安装目录不复用旧 Schema 2/4/5/6/7 的 current/previous 指针。不要复制旧 current.json、previous.json 或 process.json 到新安装。停旧服务使用原安装目录的 Stop.cmd 并核对进程身份，新启动器不接管旧 Schema 安装。

安装准备和调用示意（不代表已获得正式操作授权）：

```powershell
# 初次设置必须明确选择已迁移并核验的库；源库未迁移时会拒绝启动。
pwsh -NoProfile -File .\distribution\windows\update-local.ps1 -InstallRoot E:\steward-schema8-app -NoOpen
...
成功只输出 `databaseSchema=8`，随后退出；不创建数据库、父目录、runtime、凭据或监听器。不存在且无 sidecar 的路径允许之后正常首次初始化；已有库必须是普通文件且版本为8，Schema 0/1/2/3/4/5/6/7/未知版本、无法解读的 SQLite 文件、歧义路径或非法 sidecar 均拒绝。读取 SQLite 已提交 WAL 视图，不从可能过期的主文件头猜测版本。
...
包要求UI合同5，旧合同1/2/3/4服务不能通过纯UI更新适配；不得改号冒充兼容。首次需经用户授权升级配套 taskd；新启动器为声明 `uiPackageProtocol: 1` 的二进制包传入 `<InstallRoot>/ui`。之后使用源码内 `ui.ps1`，不再为纯 UI 改动运行 `Update-Local.cmd`：
...
- `test.ps1`（不传 `-Taskd`）、`test-local.ps1`、`test-ui-contract.ps1` 仅使用临时目录和模拟进程/资产，不操作正式安装或启动服务。
- 实际监听测试通过 `STEWARD_TEST_BIND` 指定网卡，CI 未设置时默认 `127.0.0.1`；本机验证显式设为 `172.19.10.185`，地址不可用时失败，不回退。
- Rust 实际 daemon 测试 `schema2_startup`、`database_permissions` 使用相同环境变量。
- `test-local-runtime.ps1` 会在隔离目录编译并启动临时服务；`test-ui.ps1` 使用临时数据库、runtime 和 UI 目录。执行前须核实测试地址及目标二进制，不指向正式安装。
