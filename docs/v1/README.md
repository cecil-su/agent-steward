...
状态：**Workspace / Repository 优先的独立设计基线草案**

> **版本关系：**V1 是独立方案，不是 V0 的下一版本、扩展版或迁移目标，也不自动继承 V0 的代码、数据模型、命令合同与完成状态。V0、V1 以及未来可能出现的 V2 只有在专项文档明确声明时才存在特定复用或迁移关系；否则必须分别设计、实现和验收。统一口径见仓库根目录 [README](../../README.md#版本与方案关系)。
