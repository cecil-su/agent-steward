...
- `session bind <session-id> --source <source> --external-session <id> --if-version <v>`：给已存在且未结束的本地 Session（不限制Task业务状态） 一次性绑定来源。已有同值绑定是当前版本 no-op；不同绑定拒绝。`source + externalSessionId` 全库唯一。绑定写 Task version 和 History；不改变当前 Session。
- `claim/resume`仅管理显式Session关联，不改变Task业务状态；`task status`是唯一业务状态入口。客户端的 started/resumed/idle/closed 是观测事件，绝不映射为 claim/resume/session close。失联和迟到事件不能回收执行权。
...
Schema8的`session_events`外键指向Session，不是通用Event/Actor/Operation模型。普通连接拒绝不兼容格式，不隐式迁移或重建。CLI/HTTP业务JSON envelope为3，事件输入DTO自身schemaVersion仍为1，二者不可混用。
...
- 所有业务经过Application Service；SQLite及必要同步文件工作在阻塞线程池执行；不调用Git、不读取项目源码。业务总并发8，读取最多占6，为写请求保留2个容量；读容量耗尽返回503。Cookie角色查询、授权发放与撤销使用独立4槽阻塞执行器，纯Token校验及默认本机信任不依赖认证SQLite槽。所有permit均由实际worker持有至完成，即使HTTP future取消也不提前释放；SQLite quick_check等查询没有通用执行截止时间，不能把HTTP取消当作后台SQLite已停止。保留CAS、事务与History；不再有Worktree锁、dirty检查或Git部分完成协议。
- 首次启动创建管理员与只读两份随机凭据，写入用户专属 runtime/identity 目录并跨重启保留，普通 stdout/stderr 仅显示 URL 和凭据文件路径，不显示秘密。默认以真实 TCP peer 判定本机：回环地址或与监听网卡 IP 相同的直接来源可作为管理员，不需要 Cookie 或连接码。缺少 peer、含 Forwarded/X-Forwarded-* 的请求不获得本机特权；显式请求头凭据仍按其角色处理。此模式信任本机所有用户/程序，禁止通过本机代理、隧道或端口转发暴露；代理部署必须加 `--require-local-auth`。严格模式启动时才通过本机浏览器启动器传递随机一次性连接码（120 秒有效），页面立即清除 URL fragment，再通过 `POST /api/connect` 兑换浏览器授权 Cookie（不返回长期凭据）；码只能成功使用一次，兑换遵循同样的 Host/Origin 边界，不按来源 IP 放行。`--no-open` 禁止自动打开；原生页面的其他浏览器通过申请/审批获得独立只读授权，不输入凭据；管理员保留默认本机入口和严格模式的一次性连接链接。React 回退页面及已有请求头凭据接口保留；长期凭据仍属于单独管理的访问能力，撤销某个浏览器不会轮换长期凭据；本机免登录不因 Cookie 退出或撤销而关闭。浏览器使用随机独立授权 Cookie，`HttpOnly; SameSite=Strict; Path=/api; Max-Age=2592000`（30 天），服务端在私有 identity/browser-sessions.db 只保存 Cookie 哈希、角色、凭据绑定和过期时间。普通网址、新标签页及同 origin 重启可恢复；长期凭据不进入sessionStorage/localStorage或响应正文。退出登录删除对应授权并清除 Cookie；管理员可撤销当前 origin 全部浏览器授权；长期凭据轮换也使绑定的旧授权失效。当前只提供 HTTP，Cookie 不设 Secure，不能把 LAN HTTP 视作加密连接。一次性连接码仅放入本机启动链接的 fragment、不进入查询参数或日志。
- 原生 `/credentials` 为管理员访问授权页，导航与列表/看板并列；其它页面也轮询待审批申请并弹出核对对话框。申请者和管理员均显示相同核对码；码仅作人工核对，不能登录。设备备注/浏览器说明是自报资料，IP 取真实 TCP peer，仅供辨认，不作为设备身份或授权依据。
- `POST /api/access-request` 接收严格 JSON `{ "label": "浏览器说明" }`（1–80个可打印字符），设置随机 HttpOnly Cookie，但审批前不赋予数据权限；同一未过期申请 Cookie 可恢复原申请。`GET /api/access-request` 仅返回该 Cookie 的申请状态/核对码/到期时间或已授权标识，不暴露其他申请；无 Cookie 返回 `none`。两个端点允许匿名，创建仍强制 Origin/CSRF/JSON，申请与管理请求体上限4096字节。
- 管理 API 均须管理员，读取列表也不向 reader 开放：`GET /api/access-requests` 返回 `pending/grants`，包括非秘密标识、备注、IP、浏览器说明、申请/授权/到期时间（Unix秒）及权限；`POST /api/access-requests/{id}/approve` 接收 `{ "verificationCode": "核对码" }`，事务内仅批准未过期 pending 申请，始终授予 reader；`POST /api/access-requests/{id}/revoke` 拒绝申请或撤销单个浏览器授权。管理写入包括请求头认证均强制精确 Origin 与 CSRF；不允许通过此接口撤销当前浏览器自身，退出使用 logout。重复/过期/核对码不符的批准返回409，不自动重试。
- 申请有效期10分钟，浏览器授权30天；每 origin/IP 每分钟最多4次新申请，最多64个待审批、512个未过期申请记录、256个未过期浏览器授权。限额返回429，认证存储失败返回503；撤销申请记录最多再有效60秒，过期记录在下一次新申请时清理；旧 Cookie 不可重新批准。批准后申请浏览器轮询并自动进入；在线授权同时由 SSE 和只读轮询核验，撤销后返回申请页，断线浏览器下次请求被拒。已查看/复制的数据无法收回。
- 申请记录保存在私有 `identity/browser-sessions.db` 的 `browser_access_requests` 表，Cookie只保存哈希；批准原子插入既有 `browser_sessions`，删除授权立即使 Cookie 失效。新增认证表不改变任务库Schema8、Task、History或Session，不写任务记录。启用需要配套后端与原生包，正式部署及认证库结构扩展须另行确认；不能仅升级页面到旧服务。
- 每个 API 请求都检查精确 Host，存在 Origin 时必须匹配；拒绝 null/跨 Origin 和跨站 Fetch Metadata。除可信本机直接请求外，业务 API 要求有效的请求头凭据或浏览器 Cookie（一次性连接端点使用连接码；申请/自身申请状态为受限匿名例外）。本机免登录或含 Cookie 的非 GET/HEAD 请求必须同时具有精确 Origin 与 `X-Steward-CSRF: 1`，POST 还要求 JSON Content-Type；请求头凭据若显式提供但错误，不回退到 Cookie。无 CORS 放行；静态资源禁止跨源嵌入，CSP 不允许内联脚本和第三方资源；用户内容不执行 HTML 或脚本；任务长文本可按安全 Markdown 渲染，原始 HTML 显示为文本，不加载内容中的图片或第三方资源。
...
`POST /api/commands/task-status`接收严格JSON `{"taskId":1,"expectedVersion":3,"status":"in_review"}`，允许七状态任意转换，CAS后同值no-op；不改变Session或清空资料。update仍禁止status字段；确认字段不能代替用户实际指令。详见[七状态](21-待上线任务状态.md)。

旧task-block/unblock/pending-release/continue/close、全部Worktree命令/查询、源码context/resolve及目录探测入口已移除。Task/Context不返回Repository/Worktree字段或worktreeStatus。Project源码只接受directory路径资料，不检查Git或读取源码。UI API合同5、包格式1，旧合同请求与包不自动适配。

本项目后续启动新实例须显式`--bind 172.19.10.185`，访问`http://172.19.10.185:PORT`；不可用先确认，不回退到默认回环地址。程序默认绑定行为不是本项目执行许可。

## 验证与限制

合同回归覆盖绑定、去重/冲突/墓碑、输入与容量、认证、Origin/CSRF、reader写入拒绝、CAS、七状态与Session解耦、退役接口拒绝及只读页面。入口见[测试与验收](08-测试与验收.md)和[原生适配指南](../../integrations/README.md)。
