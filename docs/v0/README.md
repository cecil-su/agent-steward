...
00–05 描述 Task/Session/存储/事务核心，06–10 描述界面与流程。当前 M4/M5 行为以 11/12 为详细合同；13 专门定义 v7 归档复制，14 定义独立 UI 发布；15 仅是参考建议与待验证计划；16 是 #34 项目/上下文开发合同，明确 Schema 4 隔离开发与尚未启用的复用边界。历史测试数量不代表当前覆盖，CI 结果须按提交读取。出现冲突应依据代码和验收证据修订合同，不能仅按文档编号推断功能已完成。
...
- [项目管理人工验收交接（待实际执行）](17-项目管理验收.md)

当前开发 Schema 为 4，仅初始化空库，拒绝 Schema 2 活动库及 Schema 3 开发库，不隐式升级；不要将开发构建直接安装到正式 CLI/Hook/taskd。JSON envelope 仍为 2。
