...
状态：已有实现，末尾保留首轮历史验收；后续功能以对应提交的测试记录为准。本文收敛 `07-实施路线图` 的后续阶段，优先于早期 API 草案；不计入 V1。本文末尾为历史验收快照，不覆盖后续全部改动。
...
- 单个 Rust Daemon 从启动参数固定数据库，提供同源静态 GUI 和 JSON API。默认绑定 `127.0.0.1`，默认端口 `43123`（`--port 0` 可显式选择临时端口）；用户可显式通过 `--bind <本机IPv4>` 绑定指定网卡地址，不允许通配、多播或广播地址，不接受请求切换数据库。Host/Origin 严格匹配实际监听地址和端口，不放宽 CORS。非回环 HTTP 不加密传输，启动时告警，使用者须限制网络访问；程序不自动修改防火墙。
...
历史开发记录使用过忽略目录下的独立追踪库与旧构建，不是用户安装前提；当前操作应显式核对数据库路径与二进制格式，不依赖开发者本机追踪文件。


### 历史验收证据（d43e0a9，2026-09-07，macOS）
...
- 真实 Chrome + 临时 Daemon：创建、领取、Checkpoint、CAS 冲突保留输入、恢复 Session、绑定/清除 Hook、Import 删除、阻塞/解除、响应丢失不重放、Worktree dirty 删除保护、上下文复制、显式关闭、移动端布局和当时的内存认证：通过。该记录不能证明后来 Cookie、本机免登录、SSE 或外置 UI 功能通过。
- 最终 review 补齐 `task notes` / HTTP notes 查询和 GUI 展示，防止备注写入后无法查看；修复提交成功但刷新失败的提示、可选关闭原因和异步列表竞态。
- 该历史记录未覆盖 Windows/Linux 和真实客户端模型运行；正文、附件、model/usage 仍为延后项，SSE 此后已实现。当前交付为通用及 Codex/pi 元数据 Hook 与本地工作台，不扩大为完整聊天归档系统。
- 当时未自动关闭 Task；该轮实现已随 d43e0a9 提交。文档不维护当前本机 Task、Git 工作区或推送状态。使用方法和测试重现见 [使用与验收](12-M4-M5使用与验收.md)。
