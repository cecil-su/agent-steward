...
fn run(db: &Path, args: &[&str]) -> Value {
    ...
    serde_json::from_slice(&output.stdout)
        .unwrap_or_else(|_| panic!("CLI output is not JSON: {:?}", output.stderr))
}
...
fn cli_profile_requires_provenance_and_cas_and_injects_read_only_context() {
    ...
    assert_eq!(
        run(&db, &["project", "create", "--name", "Profile"])["ok"],
        true
    );
    assert_eq!(
        run(&db, &["task", "create", "SOURCE", "--project", "##1"])["ok"],
        true
    );
    ...
    assert_eq!(
        run(&db, &["project", "show", "##1"])["data"]["project"]["revision"],
        2
    );
}
...
fn schema4_cli_copy_requires_explicit_confirmation_and_keeps_source_unchanged() {
    ...
    c.execute_batch(include_str!("../../application/src/migration/schema4.sql"))
        .unwrap();
    drop(c);
