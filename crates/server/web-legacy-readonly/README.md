...
- 看板及访问授权路由要求后端提供 `GET /dashboard`、`GET /credentials` 和审批 API，与首页使用相同 UI shell 和安全头。旧服务须先配套更新后端；仅更新 UI 会导致看板直接访问或刷新返回404。正式更新需单独确认后端正常重启，不涉及数据库迁移。
...
- 顶部“访问授权”链接指向 `/credentials`，仅管理员可见；管理接口亦在后端拒绝 reader/匿名访问。
- 访客页面没有凭据输入框，点击“申请访问”后显示核对码并等待。管理员打开工作台时每3秒检查新申请，弹窗展示自报浏览器说明、真实来源IP、时间及核对码，批准后仅授予该浏览器只读访问；备注/IP不是设备身份凭证。
- 管理页列出待审批申请和浏览器授权，支持拒绝、批准只读及单独撤销；当前浏览器使用退出登录，不在列表中撤销自身。本机免登录管理员入口不受浏览器授权撤销影响，严格模式保留 taskd 自动打开浏览器时的一次性连接链接；`--require-local-auth --no-open` 需已有管理员 Cookie 或通过既有管理员认证 API 建立浏览器授权，不会绕过严格模式自动批准。
- 批准后访客自动进入；撤销后 SSE 或只读轮询使页面返回申请页，后端拒绝旧Cookie的后续数据请求。断线设备下次请求被拒，已复制的数据无法收回。
- 管理写请求单飞，不自动重试；失败/冲突/结果未知暂停操作，需显式刷新核对。仅轮询不会恢复失败写操作的提交能力；迟到的管理员响应不会在退出后重新显示数据。
- 核对码不是登录凭证，Cookie明文仅经HttpOnly Set-Cookie交给申请浏览器，不展示给管理员或写入任务数据。认证库保存哈希与受限元数据；限流、过期、角色及接口见[HTTP合同](../../../docs/v0/11-Hook与HTTP合同.md)。HTTP没有传输加密，仅用于受信任网络。
- React回退与已有请求头凭据接口不改变；历史凭据产生的浏览器授权可撤销，但不等同于轮换长期凭据。正式启用需配套后端和原生包，涉及认证库结构扩展，必须独立确认，不自动部署。
...
- 传输层只允许认证/申请POST、管理员访问授权审批/撤销及管理员 `task-status`，仍拒绝创建/编辑/备注/规则等其他业务写入。规则不授予领取、推送、部署或关闭任务权限。
