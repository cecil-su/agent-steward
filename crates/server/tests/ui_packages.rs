...
async fn switches_whole_releases_and_rolls_back_without_rebuilding_router() {
    ...
    assert_eq!(status(&app).await["apiContract"], 2);
    let embedded = status(&app).await["release"].as_str().unwrap().to_owned();
    let a = package(root, "a", 2);
    let b = package(root, "b", 2);
    activate(root, &a);
...
async fn refuses_bad_packages_and_keeps_last_snapshot() {
    ...
    let root = temp.path();
    let app = fixture(root);
    let a = package(root, "good", 2);
    activate(root, &a);
    assert_eq!(status(&app).await["release"], a);
    let incompatible = package(root, "old-contract", 1);
    ...
    let corrupt = package(root, "corrupt", 2);
    fs::write(
    ...
    let response = app
        .oneshot(
            Request::builder()
                ...
                .header("x-steward-ui-contract", "1")
