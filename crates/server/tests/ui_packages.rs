...
async fn switches_whole_releases_and_rolls_back_without_rebuilding_router() {
    ...
    let a = package(root, "a", 1);
    let b = package(root, "b", 1);
    activate(root, &a);
    ...
    let response = app
        .oneshot(
            Request::builder()
                ...
                .header("x-steward-ui-contract", "0")
